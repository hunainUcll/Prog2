...
def sum_movie_duration_from_period(movies,startYear,endYear):
    total_duration = 0
    for movie in movies:
        if movie.year >= startYear and movie.year < endYear:
...
def sum_movie_duration_in_period(movies,periodStart,periodEnd):
    total_duration = 0
    for movie in movies:
        if movie.year >= periodStart and movie.year < periodEnd:
            total_duration += movie.duration
    return total_duration

...
# because if n is divisible by something larger than its square root, 
...
def is_increasing(ns):
    return all(ns[x]<=ns[x+1] for x in range(len(ns)-1))

def count_matching(xs, ys):
    return  sum(1 for x,y in list(zip(xs,ys)) if x ==y)

def weighted_sum(ns, weights):
    return sum([element[0] * element[1] for element in list(zip(ns,weights))])
